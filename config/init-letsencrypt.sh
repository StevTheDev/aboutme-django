...
docker exec -it aboutmedjango_aboutme-proxy_1 bash
echo
...
docker-compose run --rm --entrypoint "\
  ...
  rm -Rf /etc/letsencrypt/renewal/$domains.conf" aboutme-cert
echo
...
docker-compose run --rm --entrypoint "\
  certbot certonly --webroot -w /var/www/certbot \
    ...
    --agree-tos \
    --force-renewal" aboutme-cert
echo

echo "### Reloading nginx ..."
docker-compose exec aboutme-proxy nginx -s reload
